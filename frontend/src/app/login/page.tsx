...
export default function LoginPage() {
	...
	const toggleAccountType = () => {
		...
		setErrorMessage("");
		setFeedback("");
		setAccountType((prev) => (prev === "customer" ? "owner" : "customer"));
	};
						...
						<div className="grid h-full grid-cols-1 md:grid-cols-2">
							...
							<div className="flex h-full min-h-0 flex-col border-stone-800 bg-stone-950 md:border-l">
								...
								<div className="flex min-h-0 flex-1 flex-col px-5 pb-4 pt-3 sm:px-6">
									<Tabs
										className="login-auth-tabs flex min-h-0 flex-1 flex-col gap-0"
										selectedKey={mode}
										onSelectionChange={(key) => {
											if (key == null) {
												return;
											}
											setMode(String(key) as FormMode);
											setErrorMessage("");
											setFeedback("");
										}}
									>
										<Tabs.ListContainer className="shrink-0">
											<Tabs.List aria-label="Kimlik işlemleri">
												<Tabs.Tab
													className="flex-1 rounded-md px-3 py-2 text-center text-sm font-medium text-stone-200 outline-none transition data-[hovered]:bg-stone-800/90 data-[hovered]:text-stone-50 data-[selected]:bg-transparent data-[selected]:text-stone-50 data-[selected]:shadow-none data-[focus-visible]:outline data-[focus-visible]:outline-2 data-[focus-visible]:outline-offset-2 data-[focus-visible]:outline-amber-500/45"
													id="login"
												>
													Giriş yap
													<Tabs.Indicator />
